...
async function handleCreate(body) {
  ...
  if (!baseAmount || baseAmount <= 0) return NextResponse.json({ ok: false, error: 'baseAmount tidak valid' }, { status: 400 });
  ...
  let invoiceId = null;
  try {
    const { data: inv } = await supabase.from('invoices').insert({ type, nick: nick.trim(), platform, final_amount: baseAmount, payment_method: 'QRIS', details: details || {}, expires_at: expiresAt }).select('id').single();
    if (inv?.id) invoiceId = inv.id;
  } catch { }

  const { data: order, error } = await supabase.from('beta_orders').insert({ suffix, nick: nick.trim(), platform, type, base_amount: baseAmount, total_amount: totalAmount, details: details || {}, expires_at: expiresAt, invoice_id: invoiceId }).select('id, suffix, total_amount, expires_at').single();
...
async function handleNotify(request) {
  ...
  return NextResponse.json({ ok: true, rcon: rconResult.ok, nick: order.nick, type: order.type });
}

async function handleStatus(request) {
  const orderId = new URL(request.url).searchParams.get('orderId');
  if (!orderId) return NextResponse.json({ ok: false, error: 'orderId diperlukan' }, { status: 400 });
  const { data: order, error } = await supabase.from('beta_orders').select('id, status, nick, type, total_amount, expires_at, paid_at').eq('id', orderId).single();
  ...
  return NextResponse.json({ ok: true, orderId: order.id, status: order.status, nick: order.nick, type: order.type, totalAmount: order.total_amount, expiresAt: order.expires_at, paidAt: order.paid_at });
}

export async function POST(request) {
  const action = new URL(request.url).searchParams.get('action');
  if (action === 'create') { const body = await request.json().catch(() => ({})); return handleCreate(body); }
