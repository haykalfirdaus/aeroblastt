...
import { grantRank, giveKey, giveCoins, giveClaimLimit, grantCommand } from '@/api/_rcon';
...
const VALID_TYPES = ['rank', 'key', 'skill', 'claim', 'coins', 'command', 'cosmetic', 'donate'];
...
const TYPE_LABEL = { rank: '🎖️ Rank', key: '🗝️ Gacha Key', skill: '⚡ Skill Boost', claim: '🏘️ Claim Limit', coins: '🪙 Coins', command: '⌨️ Command', cosmetic: '✨ Custom Prefix' };
...
async function executeRcon(order) {
  ...
  if (type === 'claim') return giveClaimLimit(nick, details.claims);
...
async function handleCreate(body, request) {
  ...
  if (type === 'donate') {
    ...
    return NextResponse.json({ ok: true, orderId: order.id, suffix: order.suffix, totalAmount: order.total_amount, expiresAt: order.expires_at, qris: await buildDynamicQris(order.total_amount) });
  }
  ...
  if (details?.claims) productFields.push({ name: 'Claim Limit', value: Number(details.claims).toLocaleString('id-ID'), inline: true });
...
async function handleNotify(request) {
  ...
  if (order.details?.claims) productFields.push({ name: 'Claim Limit', value: Number(order.details.claims).toLocaleString('id-ID'), inline: true });
