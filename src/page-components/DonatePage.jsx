    ...
    <PageLayout>
      ...
      <div className="relative border-b border-2 border-[#1d2b1f] bg-[#f5ede0] px-4 py-10 text-center sm:px-6 lg:px-8">
        ...
        <span data-aos="fade-down" data-aos-duration="600" className="relative mb-3 inline-flex items-center gap-1.5 rounded-md border border-[#BFFF5E]/35 bg-[#BFFF5E]/10 px-3 py-1 font-mono text-[0.65rem] font-semibold uppercase tracking-[0.15em] text-[#1d2b1f]">
          <Heart size={10} className="fill-[#1d2b1f]" />Dukung Server
        </span>
      ...
      <div className="mx-auto max-w-5xl px-4 py-10 sm:px-6 lg:px-8">
        <div className="grid gap-10 lg:grid-cols-[1fr_360px]">
          ...
          <div>
            ...
            {/* PAID */}
            {step === 'paid' && (
              <div className="flex flex-col items-center justify-center gap-6 rounded-md border border-[#BFFF5E]/30 bg-[#BFFF5E]/[0.07] px-6 py-16 text-center"
                style={{ animation: 'page-wipe-in 0.4s cubic-bezier(0.22,1,0.36,1) both' }}>
                <div className="flex h-20 w-20 items-center justify-center rounded-md border-2 border-[#BFFF5E]/50 bg-[#BFFF5E]/15">
                  <CheckCircle size={38} className="text-[#1d2b1f]" />
                </div>
                <div>
                  <h2 className="font-display text-2xl font-extrabold text-[#1d2b1f]">Donasi Diterima! 🎉</h2>
                  ...
                  {loggedNick && (
                    <p className="mt-2 rounded-lg border border-[#BFFF5E]/25 bg-[#BFFF5E]/10 px-3 py-1.5 text-xs text-[#1d2b1f]">
                      Donasi atas nama <span className="font-bold">{loggedNick}</span> sudah masuk leaderboard!
                    </p>
                  )}
                </div>
                <button type="button" onClick={handleReset}
                  className="rounded-md border border-[#BFFF5E]/40 bg-[#BFFF5E]/15 px-5 py-2.5 text-sm font-semibold text-[#1d2b1f] transition-all hover:bg-[#BFFF5E]/25">
                  Donasi Lagi
                </button>
              </div>
            )}
            ...
            {/* EXPIRED */}
            {step === 'expired' && (
              <div className="flex flex-col items-center justify-center gap-5 rounded-md border border-2 border-[#1d2b1f] bg-[#fffdf9] px-6 py-14 text-center"
                style={{ animation: 'page-wipe-in 0.4s cubic-bezier(0.22,1,0.36,1) both' }}>
                ...
                <div className="flex flex-col gap-4 rounded-md border border-2 border-[#1d2b1f] bg-[#fffdf9] p-5">
                  ...
                  <div>
                    <div className="mb-1.5 flex items-center justify-between">
                      <span className="flex items-center gap-1 text-[11px] text-[#4a5e3a]"><Clock size={11} />Berlaku selama</span>
                    ...
                    <div className="h-1.5 w-full overflow-hidden rounded-md bg-[#E6E0D4]">
                      <div className={cn('h-full rounded-md transition-all duration-1000', expUrgent ? 'bg-danger' : 'bg-[#BFFF5E]')} style={{ width: `${expPct}%` }} />
                    </div>
                  </div>
                  ...
                  <button type="button" onClick={handleReset} className="text-center text-[11px] text-[#6b7f5a] transition-colors hover:text-[#4a5e3a]">
                    ← Batalkan &amp; kembali
                  </button>
                </div>
              </div>
            ...
            {step === 'form' && (
              <div style={{ animation: 'page-wipe-in 0.28s cubic-bezier(0.22,1,0.36,1) both' }}>
                ...
                {!authLoading && (
                  loggedNick ? (
                    <div className="mb-5 flex items-center gap-3 rounded-md border border-[#BFFF5E]/30 bg-[#BFFF5E]/[0.07] px-4 py-3">
                      <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-md border border-[#BFFF5E]/40 bg-[#BFFF5E]/15">
                        <Trophy size={14} className="text-[#1d2b1f]" />
                      </div>
                    ...
                    <div className="mb-5 flex items-start gap-3 rounded-md border border-2 border-[#1d2b1f] bg-[#f5ece0] px-4 py-3">
                      <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-md border border-2 border-[#1d2b1f] bg-[#f5ede0]">
                        <LogIn size={14} className="text-[#6b7f5a]" />
                      </div>
                      <div className="min-w-0">
                ...
                <div className="mb-6 flex flex-col items-center gap-2 text-center">
                  <div className="flex h-14 w-14 items-center justify-center rounded-md border border-[#BFFF5E]/35 bg-[#BFFF5E]/10">
                    <Heart size={26} className="text-[#1d2b1f]" />
                  </div>
                ...
                <div className="flex flex-col gap-4 rounded-md border border-2 border-[#1d2b1f] bg-[#fffdf9] p-5">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {QUICK_AMOUNTS.map((val) => (
                        <button key={val} type="button" onClick={() => setAmount(String(val))}
                          className={cn('rounded-md border px-3 py-1.5 text-xs font-semibold transition-all',
                            numAmount === val
                  ...
                  <div>
                    <label className="mb-1.5 block text-xs font-semibold text-[#4a5e3a]">
                      Atau ketik nominal <span className="font-normal text-[#6b7f5a]">(min. Rp 1.000)</span>
                    </label>
                  ...
                  {!loggedNick && (
                    <div>
                      <label className="mb-1.5 block text-xs font-semibold text-[#4a5e3a]">
                        Nama <span className="font-normal text-[#6b7f5a]">(opsional — tidak masuk leaderboard)</span>
                      </label>
                  ...
                  <div>
                    <label className="mb-1.5 flex items-center justify-between text-xs font-semibold text-[#4a5e3a]">
                      <span>Pesan <span className="font-normal text-[#6b7f5a]">(opsional)</span></span>
                  ...
                  {isValid && (
                    <div className="rounded-md border border-[#BFFF5E]/20 bg-[#BFFF5E]/[0.06] px-4 py-3">
                      ...
                      {willAppearOnLeaderboard && (
                        <p className="mt-1 text-[10px] font-semibold text-[#1d2b1f]">✓ Akan masuk leaderboard sebagai {loggedNick}</p>
                      )}
                    </div>
                  )}
          ...
          <div>
            <div className="sticky top-24">
              {/* Tab switcher */}
              <div className="mb-4 flex rounded-md border border-2 border-[#1d2b1f] bg-[#f5ece0] p-1 gap-1">
                <button type="button" onClick={() => setLbTab('leaderboard')}
                  className={cn('flex flex-1 items-center justify-center gap-1.5 rounded-lg py-2 text-xs font-semibold transition-all',
                    lbTab === 'leaderboard' ? 'bg-white text-[#1d2b1f] shadow-sm' : 'text-[#6b7f5a] hover:text-[#4a5e3a]')}>
                  <Crown size={11} />Leaderboard
                </button>
                <button type="button" onClick={() => setLbTab('recent')}
                  className={cn('flex flex-1 items-center justify-center gap-1.5 rounded-lg py-2 text-xs font-semibold transition-all',
                    lbTab === 'recent' ? 'bg-white text-[#1d2b1f] shadow-sm' : 'text-[#6b7f5a] hover:text-[#4a5e3a]')}>
              ...
              {/* Leaderboard */}
              {lbTab === 'leaderboard' && (
                <div className="flex max-h-[560px] flex-col gap-2 overflow-y-auto rounded-md border border-2 border-[#1d2b1f] bg-[#fffdf9] p-4">
                  {lbLoading ? (
                    ...
                    <div className="flex flex-col items-center gap-3 py-10 text-center">
                      ...
                      <div>
                        ...
                        <p className="mt-0.5 text-[11px] text-[#6b7f5a]">
                          Login di Store lalu donasi untuk tampil di sini!
                        </p>
                      </div>
                      <Link href="/store"
                        className="inline-flex items-center gap-1.5 rounded-md border border-[#BFFF5E]/40 bg-[#BFFF5E]/10 px-3 py-1.5 text-xs font-semibold text-[#1d2b1f] hover:bg-[#BFFF5E]/20">
                    ...
                    leaderboard.map((entry, i) => (
                      <div key={entry.nick}
                        className={cn('flex items-center gap-3 rounded-md border px-3 py-2.5 transition-all',
                          i === 0 ? 'border-[#BFFF5E]/40 bg-[#BFFF5E]/[0.08]'
                            : i === 1 ? 'border-2 border-[#1d2b1f] bg-[#F5F4EE]'
                              : i === 2 ? 'border-2 border-[#1d2b1f] bg-[#fffdf9]'
                                : 'border-2 border-[#1d2b1f]/60 bg-white')}>
                        {/* Rank */}
                        <div className="w-7 shrink-0 text-center">
                          {i < 3
                            ? <span className="text-lg leading-none">{MEDAL[i]}</span>
                            : <span className="text-xs font-bold text-[#6b7f5a]">{i + 1}</span>
                          }
                        </div>
                        {/* Nick */}
                        <div className="min-w-0 flex-1">
                          <p className={cn('truncate text-xs font-bold', i === 0 ? 'text-[#1d2b1f]' : 'text-[#1d2b1f]')}>{entry.nick}</p>
                          <p className="text-[10px] text-[#6b7f5a]">{entry.count}x donasi</p>
                        </div>
                        {/* Total */}
                ...
                <div className="flex max-h-[560px] flex-col gap-2 overflow-y-auto rounded-md border border-2 border-[#1d2b1f] bg-[#fffdf9] p-4">
                  {recentLoading ? (
                    ...
                    <div className="flex flex-col items-center gap-2 py-10 text-center">
                      ...
                      <p className="text-xs text-[#6b7f5a]">Belum ada donasi.<br />Jadilah yang pertama! 💚</p>
                    </div>
                  ) : (
                    recent.map((d, i) => (
                      <div key={d.id}
                        className={cn('flex flex-col gap-1 rounded-md border px-3 py-2.5',
                          i === 0 ? 'border-[#BFFF5E]/30 bg-[#BFFF5E]/[0.07]' : 'border-2 border-[#1d2b1f] bg-white')}>
                        <div className="flex items-center justify-between gap-2">
                          <div className="flex items-center gap-1.5 min-w-0">
                            {d.nick
                              ? <span className="truncate text-xs font-semibold text-[#1d2b1f]">{d.nick}</span>
                              : <span className="truncate text-xs text-[#6b7f5a] italic">{d.donor_name}</span>
                            }
                            {!d.nick && <span className="shrink-0 rounded-md border border-2 border-[#1d2b1f] px-1.5 py-0.5 text-[9px] text-[#6b7f5a]">anonim</span>}
                          </div>
                        ...
                        <p className="text-[10px] text-[#6b7f5a]">{timeAgo(d.paid_at)}</p>
                      </div>
                    ))
                  )}
                </div>
              ...
              {/* Total keseluruhan */}
              {recent.length > 0 && (
                <div className="mt-3 rounded-md border border-[#BFFF5E]/20 bg-[#BFFF5E]/[0.06] px-4 py-3">
                  <p className="text-[10px] font-semibold uppercase tracking-wide text-[#6b7f5a]">Total donasi terkumpul</p>
                  <p className="font-mono text-base font-extrabold text-[#1d2b1f]">
                    {formatRupiah(recent.reduce((s, d) => s + d.amount, 0))}
                  </p>
                  <p className="mt-0.5 text-[10px] text-[#6b7f5a]">dari {recent.length} donasi · Terima kasih! 💚</p>
                </div>
              )}
            </div>
          </div>
