...
import { COINS_QUICK_PICKS, COINS_RATE } from '@/data/coins';
...
const PICKS_DESC = [...COINS_QUICK_PICKS].sort((a, b) => b.rupiah - a.rupiah);

function CoinsOrderModal({ open, onClose, initialRupiah = 0 }) {
  ...
  return (
    <>
    <Modal open={open} onClose={onClose} title="Top-Up Coins" badge="IN-GAME COINS">
      <div className="mt-6 flex flex-col gap-4">
        <CountdownBanner open={open} />
        <div className="rounded-[var(--radius-neu-lg)] bg-[#fff8f0] p-4 text-center shadow-[var(--neu-in)]">
          <p className="text-xs text-[#4a5e3a] mb-1">Kurs: Rp 1 = {COINS_RATE} Coin</p>
          <p className="font-mono text-2xl font-bold text-[#1d2b1f]">{formatNumber(coins)} <span className="text-sm font-normal text-[#4a5e3a]">Coins</span></p>
        </div>
        ...
        <DiscountCodeInput onApply={setDiscount} category="Coins" />
    ...
    <BetaPaymentModal
      ...
      productLabel={`Top-Up ${formatNumber(coins)} Coins`}
      orderPayload={{ type: 'coins', nick: (playerNick || nick).trim(), platform, baseAmount: finalPrice, details: { coins, discountPct: discount } }}
    />
    </>
  );
}
...
export function CoinsTab() {
  ...
  return (
    <>
              ...
              return (
                ...
                >
                  ...
                  <p className="text-[0.65rem] text-[#4a5e3a]">
                    {formatNumber(rupiah * COINS_RATE)} Coins
                  </p>
                </button>
              );
            })}
      ...
      <CoinsOrderModal open={modalOpen} onClose={() => setModalOpen(false)} initialRupiah={selectedRupiah} />
    </>
  );
}
