  ...
  return (
    ...
    <Modal open={open} onClose={onClose} title="Order Custom Prefix" badge="COSMETIC">
      <div className="mt-6 flex flex-col gap-4">
        ...
        <div className="rounded-xl border border-[#D8D1C0] bg-[#F5F2EA] p-3 text-center">
          <p className="mb-2 text-xs text-[#6B7F5A]">Preview Pesanan</p>
        ...
        <div className="rounded-xl border border-[#D8D1C0] bg-[#F5F2EA] p-4 text-sm text-[#4A5E3E] space-y-1">
          <p><span className="text-[#1A2E1A] font-semibold">Teks Prefix:</span> [{prefixText || 'CUSTOM'}]</p>
          <p><span className="text-[#1A2E1A] font-semibold">Warna Prefix:</span> <span className="font-mono">{prefixColor}</span></p>
          {nickColor && <p><span className="text-[#1A2E1A] font-semibold">Warna Nick:</span> <span className="font-mono">{nickColor}</span></p>}
        </div>
    ...
    <>
      <div className="mx-auto grid max-w-4xl grid-cols-1 gap-6 lg:grid-cols-2">
        ...
        <GlassCard className="p-6" style={{ animation: 'page-wipe-in 0.5s cubic-bezier(0.22,1,0.36,1) both' }}>
          ...
          {/* Live Preview */}
          <div className="mb-6 rounded-xl border border-[#D8D1C0] bg-[#EDE8DA] p-4 text-center">
            <p className="mb-3 text-xs uppercase tracking-wider text-[#6B7F5A]">Preview Prefix</p>
          ...
          <div className="mb-4">
            ...
            <p className="mt-1 text-xs text-[#6B7F5A]">{prefixText.length}/16 karakter · ditampilkan sebagai [{prefixText || '...'}]</p>
          </div>
          ...
          <div className="mb-4">
            ...
            <div className="flex items-center gap-3">
              <input type="color" value={prefixColor} onChange={(e) => setPrefixColor(e.target.value)} className="h-10 w-12 shrink-0 cursor-pointer rounded-lg border border-[#D8D1C0] bg-transparent p-1" />
          ...
          <CheckboxField checked={addNickColor} onChange={setAddNickColor} className="mb-3">
            <span className="font-semibold text-[#1A2E1A]">Warna Nickname juga</span>
            <span className="ml-1 font-mono text-xs text-[#748F1C]">(+{formatRupiah(NICK_COLOR_ADDON)})</span>
          </CheckboxField>
          ...
          {addNickColor && (
            <div className="mb-4 ml-8">
              ...
              <div className="flex items-center gap-3">
                <input type="color" value={nickColor} onChange={(e) => setNickColor(e.target.value)} className="h-10 w-12 shrink-0 cursor-pointer rounded-lg border border-[#D8D1C0] bg-transparent p-1" />
          ...
          {/* Price */}
          <div className="mb-5 rounded-xl border border-[#D8D1C0] bg-[#F5F2EA] p-4">
            <div className="flex justify-between text-xs"><span className="text-[#4A5E3E]">Custom Prefix</span><span className="font-mono font-semibold text-[#1A2E1A]">{formatRupiah(BASE_PRICE)}</span></div>
            {addNickColor && <div className="mt-2 flex justify-between text-xs"><span className="text-[#4A5E3E]">Warna Nickname</span><span className="font-mono font-semibold text-[#748F1C]">+{formatRupiah(NICK_COLOR_ADDON)}</span></div>}
            <div className="mt-3 flex justify-between border-t border-[#D8D1C0] pt-3"><span className="font-bold text-[#1A2E1A]">Total</span><span className="font-mono text-lg font-bold text-[#748F1C]">{formatRupiah(totalPrice)}</span></div>
          </div>
        ...
        <GlassCard className="flex flex-col items-center gap-5 p-6" style={{ animation: 'page-wipe-in 0.5s cubic-bezier(0.22,1,0.36,1) both', animationDelay: '100ms' }}>
          ...
          <div className="w-full rounded-xl border border-[#B4E035]/20 bg-[#B4E035]/6 p-4 text-xs text-[#4A5E3E] space-y-1.5">
