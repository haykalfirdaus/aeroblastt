...
export function CountdownBanner({ open, discount = null }) {
  ...
  return (
    <div className="mb-4 overflow-hidden rounded-xl border border-neon-500/25 bg-neon-500/6">
      <div className="flex flex-col items-center gap-1 px-4 py-2.5 text-center">
        <p className="inline-flex items-center gap-1.5 text-[0.65rem] font-bold uppercase tracking-[0.12em] text-neon-300">
          <Flame size={11} className="text-warning" />
          Promo {tick.eventLabel} — Berakhir Dalam:
        </p>
        <div className="flex items-center gap-1.5 font-mono text-lg font-bold text-text-bright">
        ...
        <p className="text-[0.6rem] text-text-faint">Hari : Jam : Menit : Detik</p>
      </div>
    </div>
  );
}
