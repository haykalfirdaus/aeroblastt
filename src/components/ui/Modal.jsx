...
export function Modal({ open, onClose, title, subtitle, icon, badge, size = 'md', children }) {
  ...
  return (
    <div
      className={cn(
        'fixed inset-0 z-[150] transition-opacity duration-150',
    ...
    >
      <div className="fixed inset-0 bg-[#1A2E1A]/40 backdrop-blur-sm" onClick={onClose} aria-hidden="true" />

      {/* Scroll container — satu scrollbar di sini, card sendiri tidak scroll */}
      <div className="pointer-events-none fixed inset-0 flex items-start justify-center overflow-y-auto p-4 py-10">
